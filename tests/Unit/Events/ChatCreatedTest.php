...
it('sets the data', function () {
    ...
    expect($event->broadcastAs())->toEqual('chat-created');
});

it('can be dispatched', function () {
    Event::fake();
    $chat = Chat::factory()->create();
    ChatCreated::dispatch($chat->id, $chat->room_id);
    Event::assertDispatched(ChatCreated::class, function (ChatCreated $event) use ($chat) {
